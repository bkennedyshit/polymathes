...
- When a message includes an [Attached media] block, treat that file as session context first. Use its path to answer the current question, compare/search if useful, or preview/open it when asked. Do not add it to permanent media memory or run a broad media index unless the user explicitly asks to add, save, index, catalog, or remember that attachment.
